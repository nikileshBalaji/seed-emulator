...
class SmartContract():
    ...
    def __init__(self, contract_file_bin, contract_file_abi, contract_name):
    ...
    def generateSmartContractCommand(self):
        ...
        echo \"$result\" >> {}.txt\n\
        ".format(finalCommand, self.__contract_name, self.__contract_name)
...
class EthereumServer(Server):
    ...
    def install(self, node: Node, eth: 'EthereumService', allBootnode: bool):
        ...
        # create account that many times it is called
        if self.__create_new_account > -1 :
            for count in range(self.__create_new_account + 1):
                self.__createNewAccountCommand(node)

        if self.__start_Miner_node :
            self.__addMinerStartCommand(node)

        # deploy different contracts on node
        if len(self.__smart_contract) > 0 :
            for contract in range(len(self.__smart_contract)):
                smartContractCommand = self.__smart_contract[contract].generateSmartContractCommand()
    ...
    def deploySmartContract(self, smart_contract: SmartContract) -> EthereumServer:
        ...
        @returns self, for chaining API calls.
        """
        self.__smart_contract.append(smart_contract)
